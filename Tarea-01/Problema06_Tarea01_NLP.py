...
# -*- coding: utf-8 -*-
"""
Problema06_Tarea01_NLP.py

Construcción de representaciones BoW (TF y TF-IDF) usando BIGRAMAS
y selección de características con tres métricas: chi-cuadrada,
mutual information, information gain.

Uso:
  python3 Problema06_Tarea01_NLP.py --csv "/home/cesar/Descargas/MeIA.csv" \
      --text-col Review --class-col Type --top-n 20 \
      --remove-stop --save-dir out_bigrams --plots
"""
...
def get_bigram_vectorizers(remove_stop):
    """Devuelve vectorizadores TF y TF-IDF SOLO con bigramas."""
    if remove_stop:
        try:
            import nltk
    ...
    vec_tf = CountVectorizer(ngram_range=(2, 2), **kwargs)
    vec_tfidf = TfidfVectorizer(ngram_range=(2, 2), **kwargs)
...
def plot_top(df, metric, top_n, save_dir, class_name):
    ...
    plt.title(f"Top {top_n} bigramas ({metric}) - Clase: {class_name}")
    plt.tight_layout()
...
def main(args):
    ...
    # 2) Vectorización SOLO con bigramas
    vec_tf, vec_tfidf = get_bigram_vectorizers(args.remove_stop)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-dir", type=str, default="out_bigrams")
